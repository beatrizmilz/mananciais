...
#' @param ano_atual  Lógico. Se quiser obter dados de anos anteriores,
#'  usar `ano_atual = FALSE`. Se quiser obter dados do ano atual, deve
#'  ser `ano_atual = TRUE`.
#' @param download_local Lógico. Caso `download_local = TRUE`, uma cópia dos
#' dados baixados será salvo localmente.
#' @param diretorio_download Diretório onde o arquivo final deve ser salvo.
#' Ex: `"data-raw/"`
#'
#'
  ...
  function(ano,
    ...
    if (ano_atual == FALSE) {
      dias <- purrr::set_names(lubridate::as_date(
        lubridate::as_date(glue::glue("{ano}-01-01")):
        lubridate::as_date(glue::glue("{ano}-12-31"))
      ))
    } else {
      # ano_atual = TRUE
      dias <- purrr::set_names(lubridate::as_date(
