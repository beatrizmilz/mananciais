...
limpar_dados <- function(ano, df,
  ...
  dados_limpos <-
    ...
    dplyr::mutate(
      ...
      pluviometria_dia = readr::parse_number(prec_dia        , locale = readr::locale(decimal_mark = ",")),
      pluviometria_mensal  = readr::parse_number(prec_mensal        , locale = readr::locale(decimal_mark = ",")),
      pluviometria_hist = readr::parse_number(prec_hist        , locale = readr::locale(decimal_mark = ","))
    ) %>%
